    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="bg-paper rounded-2xl border border-line shadow-xs overflow-hidden">
        <div className="px-4 py-3 border-b border-line flex items-center justify-between">
          <h3 className="font-bold font-display text-sm text-ink">Configured Import Sources</h3>
        ...
        {loading ? (
          <div className="p-10 text-center text-ink-soft">
            ...
            <span className="text-xs">Loading import sources...</span>
          </div>
        ) : sources.length === 0 ? (
          <div className="p-10 text-center text-ink-soft space-y-2">
            <Globe className="w-8 h-8 text-ink-soft/40 mx-auto" />
            <p className="text-xs font-medium text-ink">No external API connectors configured</p>
            <p className="text-[11px]">Add a connector to pull products from partner suppliers or external platforms.</p>
          </div>
        ) : (
          <div className="divide-y divide-line">
            {sources.map((s) => (
              <div key={s.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-xs sm:text-sm text-ink">{s.name}</span>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-forest-soft text-forest hover:bg-forest hover:text-white text-xs font-semibold transition-all"
                  >
      ...
      {showWizard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-paper rounded-2xl border border-line shadow-2xl p-5 sm:p-6 max-w-2xl w-full space-y-4 max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handleTestConnection} className="space-y-3">
                ...
                <div className="flex justify-end pt-3 border-t border-line">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-forest text-white text-xs font-bold shadow-xs flex items-center gap-2 disabled:opacity-50"
                  >
            ...
            {step === 2 && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full px-3 py-1.5 rounded-xl bg-bg border border-line text-xs font-mono"
                    >
                ...
                <div className="flex items-center justify-between pt-3 border-t border-line">
                  ...
                  <button
                    onClick={() => setStep(3)}
                    className="px-4 py-2 rounded-xl bg-forest text-white text-xs font-bold shadow-xs flex items-center gap-1.5"
                  >
            ...
            {step === 3 && (
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-forest-soft border border-forest/20 text-xs text-forest">
                  <strong>Admin Verification Safeguard:</strong> Review mapped sample items below before saving this connector.
                </div>

                <div className="space-y-1.5 max-h-56 overflow-y-auto">
                  {sampleItems.map((item, idx) => (
                    <div key={idx} className="p-2.5 rounded-xl bg-bg border border-line text-xs space-y-0.5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-line">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-forest text-white text-xs font-bold shadow-xs flex items-center gap-1.5 disabled:opacity-50"
                  >
                    {savingSource && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
