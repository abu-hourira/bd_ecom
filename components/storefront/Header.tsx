...
export default function StorefrontHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-stone-200/90 shadow-xs w-full">
      {/* 1. Desktop Top Bar */}
      {(siteSettings.contactPhone || isStaff || brandSub) && (
        <div className="hidden sm:block bg-gradient-to-r from-[#092C15] via-[#0F4A24] to-[#092C15] text-white text-xs py-1.5 px-4 border-b border-forest-light/20">
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 font-medium truncate text-[11px] text-white/90">
              <Sparkles className="w-3 h-3 text-amber-400 shrink-0" />
              {brandSub && <span suppressHydrationWarning className="truncate">{brandSub}</span>}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3.5 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <button
            ...
            className="p-2 rounded-xl text-stone-700 hover:text-stone-950 lg:hidden border border-stone-200 cursor-pointer active:scale-95 bg-[#F8F6F2]"
          ...
          <Link href="/" className="flex items-center gap-2.5 group min-h-[36px]">
            {siteSettings.siteLogo ? (
              <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl overflow-hidden shadow-xs shrink-0 ring-1 ring-stone-900/10">
                <Image
            ...
            <div className="flex flex-col justify-center">
              {brandTitle ? (
                <span suppressHydrationWarning className="font-extrabold text-lg sm:text-xl tracking-tight text-[#163E24] leading-none truncate max-w-[140px] sm:max-w-xs font-display">
                  {brandTitle}
                </span>
              ) : (
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          <button
            ...
            className="p-2 rounded-xl text-stone-700 hover:text-stone-900 md:hidden border border-stone-200 bg-[#F8F6F2] cursor-pointer active:scale-95"
            aria-label="Search"
          >
          ...
          <Link
            ...
            className="hidden sm:flex p-2 sm:px-3.5 sm:py-2 rounded-xl border border-stone-200 hover:border-forest/40 bg-[#F8F6F2] text-stone-800 items-center gap-1.5 text-xs font-semibold transition-all cursor-pointer"
          >
          ...
          >
            ...
            <span className="min-w-4.5 h-4.5 sm:min-w-5 sm:h-5 px-1 rounded-full bg-amber-400 text-stone-950 text-[10px] sm:text-[11px] font-black flex items-center justify-center">
              {cartCount}
            </span>
          </button>
        </div>
      ...
      <nav className="hidden lg:block border-t border-stone-200/70 bg-[#FAF8F5]/80">
        <div className="max-w-7xl mx-auto px-8 flex items-center justify-center gap-6 py-2">
          <Link
            href="/products"
            className="text-xs font-bold text-stone-700 hover:text-forest transition-colors py-1 cursor-pointer"
          >
          ...
          <Link
            href="/subscription"
            className="text-xs font-bold text-amber-700 hover:text-amber-800 transition-colors py-1 cursor-pointer flex items-center gap-1 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200"
          >
          ...
          <Link
            href="/recipes"
            className="text-xs font-semibold text-stone-600 hover:text-forest transition-colors py-1 cursor-pointer flex items-center gap-1"
          >
          ...
          {navCategories.map((c: any) => (
            <Link
              ...
              className="text-xs font-semibold text-stone-600 hover:text-forest transition-colors py-1 cursor-pointer"
            >
              {c.name}
            </Link>
          ))}
        ...
        <div className="lg:hidden border-t border-stone-200 bg-white px-4 py-4 space-y-4 animate-in slide-in-from-top-2 shadow-lg">
          <nav className="flex flex-col space-y-2 text-xs font-medium text-stone-700">
            ...
            <Link
              href="/subscription"
              onClick={() => setMobileMenuOpen(false)}
              className="py-2.5 border-b border-stone-100 font-bold text-amber-800 bg-amber-50/50 px-2 rounded-lg flex items-center justify-between"
            >
